# -*- coding: utf-8 -*-
"""
Created on Wed Aug 14 13:59:44 2024
...
class GradCAMPlusPlus:
    ...
    def generate(self, input_image, target_class=None):
        ...
        # Gradients and activations have shape (batch_size, num_channels, height, width)
        ...
        alpha_den = 2 * gradients.pow(2) + torch.sum(activations * gradients.pow(3), axis=(1, 2), keepdim=True)
        alpha_den = torch.where(alpha_den != 0.0, alpha_den, torch.ones_like(alpha_den))  # To avoid division by 0
        alphas = alpha_num / alpha_den
        weights = torch.sum(alphas * torch.relu(gradients), axis=(1, 2))

        # Calculate the Grad-CAM++ map
...
#random.seed(42)
selected_images = random.sample(mask_files, 100)
